...
Для запуска vLLM в виде OpenAI-compatible endpoint существует готовый Docker-образ `vllm/vllm-openai`. Однако, для успешной работы необходимо передать ему в качестве аргумента имя модели в параметре `--model`. Поскольку DataSphere Node не поддерживает передачу параметров, напрямую использовать этот образ не получится.

Мы изготовили специальный образ `shwars/vllm`, который принимает имя модели и прочие параметры сервера через переменную окружения `VLLM_ARGS`. Вы либо можете использовать этот образ, либо изготовить такой образ самостоятельно по приведённой ниже инструкции.

### Использование готового образа

Это наиболее простой способ. Вам необходимо сделать следующее:

1. В проекте датасферы, где вы будете разворачивать ноду, задайте секрет **VLLM_ARGS**, содержащий [параметры запуска сервера vLLM](https://docs.vllm.ai/en/latest/models/engine_args.html), включая имя модели, например: `--model IlyaGusev/saiga_mistral_7b_merged --dtype=half`. Что нужно учитывать:
    - Для размещения на узлах семейства **g1.x** (NVidia V100) необходимо указывать параметр `--dtype=half`
    - Можно использовать квантизированные модели из коллекции [TheBloke](https://huggingface.co/TheBloke?search_models=AWQ), при этом указав параметр `-q awq`. Но в этом случае разворачивать ноду придётся на узлах семейства **g2.x** (NVidia A100).
    - Список [поддерживаемых моделей](https://docs.vllm.ai/en/latest/models/supported_models.html)
1. Для корректного скачивания некоторых моделей из репозитория HuggingFace необходимо установить секрет **HUGGING_FACE_HUB_TOKEN**.
1. Создайте ноду датасферы со следующими настройками:
    * Имя: <имя_ноды>
    * Тип: Docker-образ
    * Путь к образу: `shwars/vllm:latest`
    * Порт: 8000
    * Таймаут: 180 секунд
    * Инстанс: с поддержкой GPU, **g1.x** или **g2.x**

Запуск ноды может занять достаточно продолжительное время. В случае возникновения проблема (нода перейдёт в статус *UNHEALTHY*), рекомендуется посмотреть логи, чтобы понять, в чем произошла ошибка. 

Также можно смотреть логи и наблюдать за процессом разворачивания ноды. Основные этапы, которые вы должны увидеть:

- Allocating instance
- Initiating container creation
- Image pulling
- Creating/starting container
- Сообщение вида `INFO: Uvicorn running on http://0.0.0.0:8000` означает, что сервис готов к работе.

Для проверки работы в интерфейсе ноды существует раздел **запрос**. Выберите тип запроса GET, Path: `/v1/models` - вы должны получить JSON-ответ с информацией о модели.

Переходите ниже к разделу **Результат**, чтобы подробнее прочитать о том, как протестировать и использовать модель. 

### Подготовка Docker-образа

Если по каким-то причинам вам не хочется использовать сторонний Docker-образ, то Вы можете изготовить свой собственный, разместить его в реестре контейнеров Yandex Cloud, и затем создать ноду из этого контейнера. Это чуть более трудоёмкий процесс, поскольку он требует согласованной работы (а значит и взаимного доступа) нескольких облачных сервисов. Подробнее этот процесс описан ниже.
...
Запустить этот образ локально Вы сможете только на виртуальной машине с GPU, поскольку vLLM не поддерживает запуск моделей на CPU. Но можно проверить правильность сборки, попытавшись запустить образ:
```bash
VLLM_ARGS="--model IlyaGusev/saiga_mistral_7b_merged" docker run vllm
```
...
1. Создайте секреты **VLLM_ARGS** и **HUGGING_FACE_HUB_TOKEN**, как описано выше.
